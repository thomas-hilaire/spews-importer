...
export class Importer {
        ...
        this.amqpConnectionProvider
            ...
            .flatMap(reply => {
                ...
                return Observable.merge(
                    reply.channel.consume(this.config.amqp.queues.event, { noAck: false }),
                    reply.channel.consume(this.config.amqp.queues.contact, { noAck: false })
                );
            })
    ...
    private runBatchThenAck(runCount: number, amqpChannel, amqpMessages) {
        ...
        runCount++;
        logger.info("Batch %d has %d messages", runCount, amqpMessages.length);
        amqpMessages.forEach(e => {
            console.log('!!!!!!!!!!!!!!!2 ', JSON.parse(e.content.toString()));
        });
